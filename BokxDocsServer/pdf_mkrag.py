...
def makeRag():
  ...
  vectorstore = Chroma.from_documents(documents=splits[:25],persist_directory="./chroma_db", embedding=OVHCloudEmbeddings(model_name=_OVH_AI_ENDPOINTS_EMBEDDING_MODEL_NAME, access_token=_OVH_AI_ENDPOINTS_ACCESS_TOKEN))
  print(vectorstore)
  ...
  if len(splits)>25:
    ...
    for i in range(batch_size, len(splits), batch_size):
        ...
        print(i,"-->",batch_size)
  #vectorstore.persist()
  print(f"[VECTOR_LOG] Finished saving vectors {vectorstore._collection.count()}")
